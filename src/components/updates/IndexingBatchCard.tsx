...
export function IndexingFileCard({ file }: { file: PendingIndexingFile }) {
  return (
    <Card className="border-amber-200 bg-amber-50/40 dark:border-amber-900 dark:bg-amber-950/20">
      <div className="flex items-start gap-3">
        <div className="mt-0.5 shrink-0">
        ...
        <div className="min-w-0 flex-1">
          ...
          <Link
            ...
            className="text-sm text-blue-600 hover:underline dark:text-blue-400"
          >
            {file.clientName} · {file.projectName}
          </Link>
          <p className="mt-2 text-xs text-amber-800 dark:text-amber-300">
            {file.label} — Sunny updates appear after indexing finishes.
          </p>
        </div>
      </div>
    </Card>
...
export function IndexingStatusList({
  batches,
  files,
}: {
  batches: ActiveUploadBatch[];
  files: PendingIndexingFile[];
}) {
  if (!batches.length && !files.length) return null;

  return (
    <div className="space-y-4">
      {files.map((file) => (
        <IndexingFileCard key={file.fileId} file={file} />
      ))}
      {batches.map((batch) => (
...
export function IndexingBatchList({ batches }: { batches: ActiveUploadBatch[] }) {
  return <IndexingStatusList batches={batches} files={[]} />;
}
